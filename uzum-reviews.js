...
// ─── Запустить планировщик (каждые N минут) ───────────────
function startScheduler(chatId, token, lang, intervalMs) {
  if (!intervalMs) intervalMs = 30 * 60 * 1000; // 30 минут
  console.log("[Reviews] Scheduler started for chat " + chatId);
  // Первый запуск сразу
  runReviewCycle(chatId, token, lang);
  // Затем по расписанию
...
// ─── Одобрить ответ вручную ───────────────────────────────
async function approveReply(chatId, token, reviewId, replyText) {
  const result = await postReply(token, reviewId, replyText);
  if (result.success) {
    await notify(chatId, "✅ Ответ на отзыв " + reviewId + " отправлен!");
  } else {
    await notify(chatId, "❌ Не удалось отправить. Скопируйте ответ вручную.");
  }
