...
// /setuzumtoken — сохранить Bearer токен для отзывов
bot.onText(/\/setuzumtoken (.+)/, async (msg, match) => {
  ...
  try {
    // Проверяем токен — пробуем получить отзывы
    const resp = await axios.post(
      "https://api-seller.uzum.uz/api/seller/product-reviews?page=0&size=1",
      { filter: "ALL" },
      {
        headers: {
    ...
    const count = resp.data?.payload?.length || 0;
    bot.sendMessage(id,
      "✅ *Токен подключён!*\n\n" +
      "🔍 Проверка прошла успешно\n" +
      "📝 Найдено отзывов без ответа: *" + (resp.data?.payload?.filter(r => !r.replyStatus)?.length || count) + "*\n\n" +
    ...
    if (status === 401 || status === 403) {
      bot.sendMessage(id,
        "❌ *Токен недействителен*\n\nПолучите новый токен:\n📖 Инструкция → 🔑 Получить токен Uzum",
        { parse_mode: "Markdown", ...mainKb(id) }
      );
    } else {
...
// /approve REVIEW_ID — одобрить ответ на негативный отзыв
