...
// Официальный API ключ Uzum Seller
const UZUM_API_KEY = process.env.UZUM_API_KEY || "";

// Заголовки для официального Uzum Seller API
// Uzum использует схему авторизации "DBS" (из Swagger)
function uzumHeaders() {
  return {
    "Authorization": UZUM_API_KEY ? `DBS ${UZUM_API_KEY}` : undefined,
...
// Базовый URL официального Seller API (из Swagger документации)
...
async function handleShopAdd(chatId, text) {
  ...
  if (s.step === "id") {
    // Принимаем slug (название из URL) или числовой ID
    const shopSlug = text.trim().replace(/\s+/g, "");
    if (!shopSlug) {
      bot.sendMessage(chatId, "❌ Введите slug магазина. Попробуйте ещё раз:");
      return true;
    }
    ...
    try {
      ...
      // Uzum API использует slug из URL, не числовой ID
      const endpoints = [
        `https://api.uzum.uz/shop/${shopSlug}`,
        `https://api.uzum.uz/api/shop/${shopSlug}`,
        `https://api.uzum.uz/api/v2/main/search/shops?title=${encodeURIComponent(shopSlug)}&size=1`,
      ];
      for (const url of endpoints) {
        try {
          const resp = await axios.get(url, { timeout: 8000, headers });
          const d = resp.data;
          shopName = d?.payload?.title || d?.payload?.shopName ||
                     d?.title || d?.name || d?.shopName ||
                     d?.data?.title || d?.data?.name ||
                     (Array.isArray(d?.payload) && d.payload[0]?.title) || null;
          if (shopName) break;
        } catch (_) { continue; }
      }
      ...
      if (!shopName) {
        ...
        bot.sendMessage(chatId,
          `⚠️ Не удалось найти магазин автоматически.\n\nВведите *название магазина* вручную:`,
      ...
      bot.sendMessage(chatId,
        `✅ *Магазин найден!*\n\nНажмите кнопку чтобы добавить:`,
        {
          parse_mode: "Markdown",
          reply_markup: { inline_keyboard: [[
            { text: `🏪 ${shopName}`, callback_data: "mp_confirm_shop" }
          ]]},
        }
      );
    } catch (e) {
      ...
      waitingShop.set(chatId, s);
      bot.sendMessage(chatId,
        `⚠️ Не удалось проверить. Введите название магазина вручную:`,
...
const UZUM_CONNECT_URL = process.env.UZUM_CONNECT_URL || "https://xpertwolfeth.github.io/Nexus-Asistant/uzum-connect.html";
...
function showInstruction(chatId) {
  ...
  if (uz) {
    bot.sendMessage(chatId,
      ...
      "4️⃣ Nusxa oling va botga yuboring: /setuzumtoken TOKEN\n\n" +
    ...
    bot.sendMessage(chatId,
      ...
      "4️⃣ Скопируйте и отправьте боту: /setuzumtoken TOKEN\n\n" +

      "*🛒 Добавить магазин вручную:*\n" +
      "🛒 Маркетплейсы → ➕ Добавить магазин → 3 шага\n\n" +

      "*📋 Команды:*\n" +
      "`/setuzumtoken ТОКЕН` — токен для отзывов Uzum\n" +
...
// /setuzumtoken — сохранить токен и показать магазины
bot.onText(/\/setuzumtoken (.+)/, async (msg, match) => {
  if (msg.chat.type !== "private") return;
  const id    = msg.chat.id;
  ...
  // 1. Сохраняем токен
  u.uzumBearerToken = token;
  saveUser(id, u);

  const loadMsg = await bot.sendMessage(id, "🔍 Токен сохранён! Загружаю ваши магазины...");

  try {
    // 2. Получаем магазины через официальный API
    let shops = [];
    if (UZUM_API_KEY) {
      try {
        const r = await axios.get(UZUM_SELLER_API + "/v1/shops", {
          headers: uzumHeaders(), timeout: 10000,
        });
        shops = r.data?.payload || r.data?.content || r.data || [];
        if (!Array.isArray(shops)) shops = [];
      } catch (_) {}
    }

    // 3. Если официальный API не дал магазины — извлекаем из отзывов
    if (!shops.length) {
      try {
        const r = await axios.post(
          "https://api-seller.uzum.uz/api/seller/product-reviews?page=0&size=20",
          { filter: "ALL" },
          {
            headers: {
        ...
        const reviews = r.data?.payload || [];
        const shopMap = {};
        reviews.forEach(rev => {
          if (rev.shop && rev.shop.id) {
            shopMap[rev.shop.id] = { id: rev.shop.id, title: rev.shop.title || "Магазин " + rev.shop.id };
          }
        });
    ...
    await bot.deleteMessage(id, loadMsg.message_id);

    if (!shops.length) {
      // Токен принят но магазины не загрузились
      bot.sendMessage(id,
        "✅ *Токен сохранён!*\n\n" +
        "Магазины не удалось загрузить автоматически.\n" +
        "Включите автоответчик вручную:\n" +
    ...
    // 4. Показываем кнопки магазинов
    const btns = shops.map(s => [{
      text: "🏪 " + (s.title || s.name || "Магазин " + s.id),
      callback_data: "start_reviews_" + s.id + "_" + encodeURIComponent(s.title || s.name || "Магазин")
    }]);
    btns.push([{ text: "✅ Все магазины сразу", callback_data: "start_reviews_all" }]);

    bot.sendMessage(id,
      "✅ *Токен подключён!*\n\nВыберите для какого магазина включить автоответчик на отзывы:",
      { parse_mode: "Markdown", reply_markup: { inline_keyboard: btns } }
    );

  } catch (e) {
    console.error("setuzumtoken error:", e.message);
    await bot.deleteMessage(id, loadMsg.message_id);
    bot.sendMessage(id,
      "✅ *Токен сохранён!*\n\n" +
...
bot.onText(/\/approve (\d+) ?(.*)/, async (msg, match) => {
  ...
  const id = msg.chat.id;
  const u  = getUser(id);
  if (!u.uzumBearerToken) {
    return bot.sendMessage(id, "❌ Сначала подключите токен командой /setuzumtoken", mainKb(id));
  }
  const reviewId = match[1];
  const customText = match[2]?.trim();

  const st = await bot.sendMessage(id, "📤 Отправляю ответ...");
  try {
    if (customText) {
      await uzumReviews.approveReply(id, u.uzumBearerToken, reviewId, customText);
    } else {
      bot.sendMessage(id, "❌ Укажите текст ответа: /approve " + reviewId + " Ваш текст ответа", mainKb(id));
      await bot.deleteMessage(id, st.message_id);
    }
    await bot.deleteMessage(id, st.message_id);
  } catch (e) {
    await bot.deleteMessage(id, st.message_id);
    bot.sendMessage(id, "❌ Ошибка отправки: " + e.message, mainKb(id));
  }
});
...
bot.on("callback_query",async(q)=>{
  ...
  if(d==="mp_add"){
    if (UZUM_API_KEY) {
      // Есть API ключ — загружаем магазины автоматически
      const loadMsg = await bot.sendMessage(id, "🔍 Загружаю ваши магазины...");
      try {
        const shops = await fetchUzumShops();
        await bot.deleteMessage(id, loadMsg.message_id);

        if (shops && (Array.isArray(shops) ? shops.length : shops?.shops?.length)) {
          const shopList = Array.isArray(shops) ? shops : (shops.shops || []);
          const btns = shopList.map(s => [{
            text: "🏪 " + (s.title || s.name || s.shopName || "Магазин " + s.id),
            callback_data: "mp_addshop_" + s.id + "_" + encodeURIComponent(s.title || s.name || "Магазин")
          }]);
          bot.sendMessage(id,
            "*🏪 Ваши магазины на Uzum:*\n\nВыберите магазин для добавления:",
            { parse_mode:"Markdown", reply_markup:{ inline_keyboard: btns } }
          );
          return;
        }
    ...
    // Нет API ключа или магазины не загрузились — ручной режим
    bot.sendMessage(id,
      "*🏪 Добавление магазина*\n\n*Шаг 1 из 3 — Добавьте сотрудника*\n\nЗайдите в:\n*Uzum Sellers → Настройки → Сотрудники → Добавить сотрудника*\n\nНомер: 📞 *" + EMPLOYEE_PHONE + "*\nРоль: *Работник ЦПТ*\n\n💡 Или добавьте API ключ командой /setuzumapi KEY для автоматического добавления магазинов\n\nПосле добавления нажмите кнопку:",
      {
        parse_mode:"Markdown",
        reply_markup:{ inline_keyboard:[[
  ...
  // Добавление магазина из API списка
  if(d.startsWith("mp_addshop_")){
    const parts = d.replace("mp_addshop_","").split("_");
    const shopId = parts[0];
    const shopName = decodeURIComponent(parts.slice(1).join("_"));
    const u2 = getUser(id); u2.shops = u2.shops||[];
    // Проверяем не добавлен ли уже
    if (u2.shops.find(s => String(s.shopId) === String(shopId))) {
      return bot.sendMessage(id, "⚠️ Этот магазин уже добавлен!", { ...mainKb(id) });
    }
    u2.shops.push({ id:Date.now(), name:shopName, shopId, active:true, reviewsEnabled:false, addedAt:new Date().toISOString() });
    saveUser(id, u2);
    bot.sendMessage(id,
      "✅ *Магазин \"" + shopName + "\" добавлен!*\n\nID: `" + shopId + "`\n\n" +
      "Теперь доступны:\n• ⚠️ Остатки товаров (Low Stock)\n• 📈 Аналитика продаж\n• 📦 Список товаров\n\n" +
      "⭐ Для ответов на отзывы используйте раздел Маркетплейсы → Отзывы",
      { parse_mode:"Markdown", ...mainKb(id) }
    );
    return;
  }
  ...
  if(d.startsWith("mp_rev_toggle_")){
    ...
    if(!u.uzumBearerToken){
      return bot.sendMessage(id,
        "❌ Сначала подключите токен отзывов!\n\n/setuzumtoken ВАШ_ТОКЕН\n\nИли нажмите 📖 Инструкция → 🔑 Получить токен",
        {parse_mode:"Markdown",...mainKb(id)}
      );
    }
    u.shops[idx].reviewsEnabled=!u.shops[idx].reviewsEnabled;
    u.reviewsActive=u.shops.some(s=>s.reviewsEnabled);
    saveUser(id,u);
    const s=u.shops[idx];
    const icon=s.reviewsEnabled?"✅":"⛔";
    const status=s.reviewsEnabled?"включён":"выключен";
    // Запускаем или останавливаем планировщик
    toggleReviewScheduler(id, u.reviewsActive);
    bot.sendMessage(id,
      icon+" *Автоответчик для \""+s.name+"\" "+status+"*\n\n"+
      (s.reviewsEnabled?"🤖 Бот начинает проверять отзывы каждые 30 минут":"⏹ Автоответы остановлены"),
      {parse_mode:"Markdown",...mainKb(id)}
    );
    return;
  }
  ...
  // Запуск автоответчика для конкретного магазина
  if (d.startsWith("start_reviews_")) {
    const u2 = getUser(id);
    if (!u2.uzumBearerToken) {
      return bot.sendMessage(id, "❌ Токен не найден. Отправьте /setuzumtoken ТОКЕН", mainKb(id));
    }
    if (d === "start_reviews_all") {
      // Включить для всех магазинов
      (u2.shops || []).forEach(s => { s.reviewsEnabled = true; s.active = true; });
      u2.reviewsActive = true;
      saveUser(id, u2);
      toggleReviewScheduler(id, true);
      bot.sendMessage(id,
        "🤖 *Автоответчик запущен для всех магазинов!*\n\n" +
        "✅ Бот проверяет отзывы каждые 30 минут\n" +
        "⭐⭐⭐⭐⭐ 4-5★ — отвечает автоматически\n" +
        "⭐ 1-3★ — присылает вам на одобрение",
        { parse_mode: "Markdown", ...mainKb(id) }
      );
    } else {
      const parts = d.replace("start_reviews_", "").split("_");
      const shopId = parts[0];
      const shopName = decodeURIComponent(parts.slice(1).join("_"));
      u2.shops = u2.shops || [];
      // Добавить магазин если не добавлен
      if (!u2.shops.find(s => String(s.shopId) === String(shopId))) {
        u2.shops.push({ id: Date.now(), name: shopName, shopId, active: true, reviewsEnabled: true, addedAt: new Date().toISOString() });
      } else {
        const shop = u2.shops.find(s => String(s.shopId) === String(shopId));
        shop.reviewsEnabled = true; shop.active = true;
      }
      u2.reviewsActive = true;
      saveUser(id, u2);
      toggleReviewScheduler(id, true);
      bot.sendMessage(id,
        ...
        "⭐⭐⭐⭐⭐ 4-5★ — отвечает автоматически\n" +
        "⭐ 1-3★ — присылает вам на одобрение\n\n" +
        "_Первая проверка запущена прямо сейчас!_",
        { parse_mode: "Markdown", ...mainKb(id) }
      );
    }
    return;
...
// ─── UZUM SELLER API — официальные запросы ──────────────────────────────────

// ─── Официальный Uzum Seller API (эндпоинты из Swagger документации) ──────────

// GET /v1/shops — список магазинов продавца
async function fetchUzumShops() {
  if (!UZUM_API_KEY) return null;
  try {
    const resp = await axios.get(`${UZUM_SELLER_API}/v1/shops`, {
      headers: uzumHeaders(), timeout: 10000,
    });
    return resp.data?.payload || resp.data?.content || resp.data || null;
  } catch (e) {
    console.error("fetchUzumShops:", e.response?.status, e.message);
    return null;
  }
}

// GET /v2/fbs/sku/stocks — остатки товаров по SKU
async function fetchUzumStock(skuIds) {
  if (!UZUM_API_KEY) return null;
  try {
    const resp = await axios.get(`${UZUM_SELLER_API}/v2/fbs/sku/stocks`, {
      headers: uzumHeaders(),
    ...
    console.error("fetchUzumStock:", e.response?.status, e.message);
    return null;
  }
}

// GET /v2/fbs/orders — заказы продавца
async function fetchUzumOrders(page = 0, size = 20) {
  if (!UZUM_API_KEY) return null;
  try {
    const resp = await axios.get(`${UZUM_SELLER_API}/v2/fbs/orders`, {
      headers: uzumHeaders(),
      params: { page, size },
      timeout: 10000,
    });
    ...
    console.error("fetchUzumOrders:", e.response?.status, e.message);
    return null;
  }
}

// GET /v1/finance/orders — аналитика продаж
async function fetchUzumFinance(dateFrom, dateTo) {
  if (!UZUM_API_KEY) return null;
  try {
    const resp = await axios.get(`${UZUM_SELLER_API}/v1/finance/orders`, {
      headers: uzumHeaders(),
      params: { dateFrom, dateTo, page: 0, size: 50 },
      timeout: 10000,
    });
    return resp.data?.payload || resp.data?.content || resp.data || null;
  } catch (e) {
    console.error("fetchUzumFinance:", e.response?.status, e.message);
    return null;
  }
}

// GET /v1/product/shop/{shopId} — товары магазина
async function fetchUzumProducts(shopId, page = 0) {
  if (!UZUM_API_KEY) return null;
  try {
    const resp = await axios.get(`${UZUM_SELLER_API}/v1/product/shop/${shopId}`, {
      headers: uzumHeaders(),
      params: { page, size: 50 },
      timeout: 10000,
    });
    ...
    console.error("fetchUzumProducts:", e.response?.status, e.message);
    return null;
  }
}

// ВАЖНО: Отзывы отсутствуют в официальном Uzum Seller API
// Для ответов на отзывы используется отдельная авторизация через браузерный токен
...
// ─── Автоответчик на отзывы Uzum ──────────────────────────
const uzumReviews = require("./uzum-reviews");
uzumReviews.init(getUser, saveUser);

// Хранилище активных планировщиков отзывов
const reviewSchedulers = new Map(); // chatId → intervalId

// Запустить/остановить планировщик для пользователя
function toggleReviewScheduler(chatId, enable) {
  const u = getUser(chatId);
  if (enable && u.uzumBearerToken) {
    if (reviewSchedulers.has(chatId)) {
      clearInterval(reviewSchedulers.get(chatId));
    }
    const intervalId = uzumReviews.startScheduler(chatId, u.uzumBearerToken, u.lang || "ru");
    reviewSchedulers.set(chatId, intervalId);
    console.log(`[Reviews] Started for ${chatId}`);
  } else {
    if (reviewSchedulers.has(chatId)) {
      clearInterval(reviewSchedulers.get(chatId));
      reviewSchedulers.delete(chatId);
      console.log(`[Reviews] Stopped for ${chatId}`);
    }
  }
}

// Восстановить активные планировщики при перезапуске
setTimeout(() => {
  const db = loadDB();
  Object.entries(db.users).forEach(([chatId, u]) => {
    if (u.uzumBearerToken && u.reviewsActive) {
      toggleReviewScheduler(chatId, true);
      console.log(`[Reviews] Restored scheduler for ${chatId}`);
    }
  });
}, 3000);
