...
function uzumHeaders() {
  return {
    "Authorization": UZUM_API_KEY ? `Bearer ${UZUM_API_KEY}` : undefined,
    "X-Api-Key":     UZUM_API_KEY || undefined,
...
// Базовый URL официального Seller API
const UZUM_SELLER_API = "https://api-seller.uzum.uz/api/seller";
...
// Получить отзывы магазина (официальный API)
async function fetchUzumReviews(page = 0, size = 20) {
  if (!UZUM_API_KEY) return null;
  try {
    const resp = await axios.get(`${UZUM_SELLER_API}/reviews`, {
      headers: uzumHeaders(),
      params: { page, size, isAnswered: false },
      timeout: 10000,
    });
    return resp.data?.payload || resp.data?.content || resp.data || null;
  } catch (e) {
    console.error("fetchUzumReviews:", e.response?.status, e.message);
    return null;
  }
}

// Ответить на отзыв (официальный API)
async function postUzumReply(reviewId, replyText) {
  if (!UZUM_API_KEY) return false;
  try {
    await axios.post(`${UZUM_SELLER_API}/reviews/${reviewId}/reply`, {
      text: replyText,
    }, {
      headers: uzumHeaders(),
      timeout: 10000,
    ...
    console.error("postUzumReply:", e.response?.status, e.message);
    return false;
  }
}

// Получить остатки товаров (официальный API)
async function fetchUzumStock() {
  if (!UZUM_API_KEY) return null;
  try {
    const resp = await axios.get(`${UZUM_SELLER_API}/stocks`, {
      headers: uzumHeaders(),
      params: { page: 0, size: 50 },
      timeout: 10000,
    });
    return resp.data?.payload || resp.data?.content || null;
  } catch (e) {
    console.error("fetchUzumStock:", e.message);
    return null;
  }
}

// Получить аналитику продаж (официальный API)
async function fetchUzumAnalytics(dateFrom, dateTo) {
  if (!UZUM_API_KEY) return null;
  try {
    const resp = await axios.get(`${UZUM_SELLER_API}/analytics/sales`, {
      headers: uzumHeaders(),
    ...
    console.error("fetchUzumAnalytics:", e.message);
    return null;
  }
}
